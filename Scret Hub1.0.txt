...
getgenv()['Check_Setting'] = function(Name)
    if not _G.Dis and not isfolder('Secret Hub') then
        makefolder('Secret Hub')
    end
    if not _G.Dis and not isfile('Secret Hub/'..Name..'.json') then
        writefile('Secret Hub/'..Name..'.json',JsonEncode(_G.Setting_table))
    end
end
getgenv()['Get_Setting'] = function(Name)
    if not _G.Dis and isfolder('Secret Hub') and isfile('Secret Hub/'..Name..'.json') then
        _G.Setting_table = JsonDecode(readfile('Secret Hub/'..Name..'.json'))
...
local win = library:Window("Secret",[[Hub]],[[Version : Premium ]],"9606070311",Enum.KeyCode.RightControl)
